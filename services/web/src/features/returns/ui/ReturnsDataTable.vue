...
import type { ReturnsTableRow } from '@/features/returns/ui/returns-table-columns'
...
const emit = defineEmits<{
  (e: 'restock', returnId: number): void
...
const columns = computed(() =>
  buildReturnsTableColumns({
    canRestock: props.canRestock,
    onRestock: (id) => emit('restock', id),
  }),
)
    ...
    >
      <div class="flex items-center gap-1 md:justify-self-start">
        ...
        <Select v-model="perPageModel">
          <SelectTrigger size="sm" class="w-20 min-w-20 px-2 text-sm" data-test="returns-per-page">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
